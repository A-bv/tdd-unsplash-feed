...
/// `UnsplashFeed.makeRemoteFeedLoader` — endpoint + URLSession client + loader +
...
final class UnsplashFeedIntegrationTests: XCTestCase {
    ...
    func test_makeRemoteFeedLoader_deliversMappedImagesThroughFullStack() async throws {
        ...
        let sut = UnsplashFeed.makeRemoteFeedLoader(accessKey: "TEST_KEY", session: URLProtocolStub.makeSession())
